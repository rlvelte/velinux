package templates

import "path/filepath"

func GenerateKitty(data ThemeColorData, cfgDir string) error {
	const tmpl = `# Kitty theme - generated by vlx-themes

# --- Core colors ---
background {{.Base}}
foreground {{.Text}}
selection_background {{.Surface1}}
selection_foreground {{.Text}}

# --- Cursor ---
cursor {{.Accent}}
cursor_text_color {{.Base}}
cursor_shape beam

# --- URL ---
url_color {{.Accent}}
url_style curly

# --- Window borders ---
active_border_color {{.Primary}}
inactive_border_color {{.Surface1}}
bell_border_color {{.Warning}}

# --- Tab bar ---
active_tab_background {{.Primary}}
active_tab_foreground {{.OnPrimary}}
inactive_tab_background {{.Surface0}}
inactive_tab_foreground {{.Subtext}}
tab_bar_background {{.Mantle}}
tab_bar_style powerline
tab_powerline_style slanted

# --- Window title ---
wayland_titlebar_color {{.Base}}

# Black
;color0 {{.Base}}
color0 {{.Crust}}
color8 {{.Surface1}}

# Red
color1 {{.Error}}
color9 {{.Error}}

# Green
color2 {{.Success}}
color10 {{.Success}}

# Yellow
color3 {{.Warning}}
color11 {{.Warning}}

# Blue
color4 {{.Primary}}
color12 {{.PrimarySubtle}}

# Magenta
color5 {{.Accent}}
color13 {{.AccentLight}}

# Cyan
color6 {{.Info}}
color14 {{.InfoSubtle}}

# White
color7 {{.Subtext}}
color15 {{.Text}}

# --- Extended colors ---
mark1_foreground {{.Base}}
mark1_background {{.Primary}}

mark2_foreground {{.Base}}
mark2_background {{.Secondary}}

mark3_foreground {{.Base}}
mark3_background {{.Accent}}
`
	return write(filepath.Join(cfgDir, "kitty", "theme.conf"), tmpl, data)
}
